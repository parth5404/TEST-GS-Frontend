  ...
  return (
    <div>
      <Link
        ...
        ${matchRoute(data.path) ? 'text-white bg-blue-600' : 'text-gray-300 hover:text-white hover:bg-gray-800'}`}
      >
        <span className={`absolute left-0 top-0 h-full w-[0.15rem] bg-blue-400 ${matchRoute(data.path) ? ' opacity-100 ' : 'opacity-0'}`} >
        </span>
